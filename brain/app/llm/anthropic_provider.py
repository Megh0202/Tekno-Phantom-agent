    ...
    async def plan_task(self, task: str, max_steps: int) -> dict[str, Any]:
        ...
        text = await self._messages_call(
            system=(
                ...
                "steps may only use these step types: navigate, click, type, select, drag, scroll, wait, handle_popup, verify_text, verify_image, hover, double_click, right_click, press_key, upload, manage_tab, manage_window, fill_prompt. "
                ...
                "SEMANTIC TARGET CONTRACT: For every click, type, select, hover, double_click, right_click, and upload step include a 'target' object "
                ...
                "(e.g. 'email', 'password', 'confirm password', 'submit button', 'country', 'Point Me button'). "
                ...
                "or accessible name the user would read (e.g. 'Email Address', 'Sign In', 'Country', 'Point Me'). "
                ...
                "For press_key steps: include 'key' field (e.g. 'Enter', 'Tab', 'Escape', 'Control+A'). Optionally include 'target' if a specific element needs focus first. "
                "For manage_tab steps: include 'action' ('open'/'close'/'switch') and optionally 'url', 'index', or 'title'. "
                "For manage_window steps: include 'action' ('resize'/'maximize'/'fullscreen') and optionally 'width'/'height'. "
                "For fill_prompt steps: include 'value' (text to type into the dialog) and 'policy' ('accept'/'dismiss'). "
                "For upload steps: include 'target' (the file input element) and 'file_path' (absolute path to the file). "
            ),
            user=(
                f"Task: {task}\n"
                ...
                "Include src_step in every step. "
                "For every interaction step, always include a semantic target object. "
                "Never emit selector:\"\". "
                "Never emit an interaction step without either a target or a valid selector. "
                "Prefer semantic targets over CSS selectors."
            ),
            max_tokens=8192,
        )
